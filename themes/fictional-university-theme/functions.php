...
register_block_type( 'prince/event-block', [
...
register_block_type( 'prince/professor-block', [
    'render_callback' => 'render_professors_block',
  ] );
  
  function render_professors_block( $attributes ) {
    // $events = get_posts(
    //   [
...
function page_banner( $args = NULL ) { // NULL sets args as optional instead of required
    ...
    if( !$args['photo'] ) {
        if( get_field( 'page_banner_background_image') AND !is_archive() AND !is_home() ) {
            ...
            $args['photo'] = get_theme_file_uri( 'images/ocean.jpg' ); // image url will break on AWS
        }
    }
?>
...
function university_files() {
    ...
    wp_enqueue_script( 'googleMap', '//maps.googleapis.com/maps/api/js?key=AIzaSyADEB0sF0iFTk4dY71AbyXIZUPLhyrIfmI', NULL, '1.0', true);
...
function university_map_key( $api ) {
    $api['key'] = 'AIzaSyADEB0sF0iFTk4dY71AbyXIZUPLhyrIfmI';
    return $api;
}
...
add_filter( 'login_headerurl', 'our_head_url' );

function our_head_url() {
